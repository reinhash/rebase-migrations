...
fn run_rebase(path: &str, dry_run: bool, all_dirs: bool) -> PyResult<()> {
    migration::project::rebase_apps(path, dry_run, all_dirs)
        .map_err(PyErr::new::<PyRuntimeError, _>)
}

#[pyfunction]
#[pyo3(signature = (app_path, dry_run=false))]
/// Fix Django migration conflicts during git rebase.
///
/// Args:
...
/// Raises:
///     RuntimeError: If the migration rebase operation fails
fn run_rebase_for_one_app(app_path: &str, dry_run: bool) -> PyResult<()> {
    let app_path = Path::new(app_path);
    migration::project::rebase_app(app_path, dry_run).map_err(PyErr::new::<PyRuntimeError, _>)
}
